...
const Mfake = fs.readFileSync ('./media/Menu.jpg')
const Mthumb = fs.readFileSync('./media/Menu.jpg')
const timeWib = moment.tz('Asia/Jakarta').format('DD/MM')

//𝙁𝘼𝙆𝙀 𝙉𝙔𝘼 𝘽𝘼𝙉𝙂
PutraXdgamteng ='PutraXd Ofc☕︎'
fake = 'PutraXd Ofc☕︎'
//Ganti No Owner Ok
const Tag = "6283842942635"		
...
let imagePutraXd =  fs.readFileSync("./media/Menu.jpg")
let fakeimage = fs.readFileSync("./media/Menu.jpg")
let imgcel = fs.readFileSync("./media/wpmobile.png")
...
module.exports = PutraXd = async (PutraXd, mek) => {
	try {
        ...
        const botNumber = PutraXd.user.jid
        const ownerNumber = setting.ownerNumber
		const ownerName = setting.ownerName
		const botName = setting.botName
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? PutraXd.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const totalchat = await PutraXd.chats.all()
		const groupMetadata = isGroup ? await PutraXd.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? PutraXd.user.jid : PutraXd.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? PutraXd.user.name : conts.notify || conts.vname || conts.name || '-'
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = PutraXd.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "Created By PutraXd Ofc","listType": "SINGLE_SELECT","sections": list}}, {})
            return PutraXd.relayWAMessage(po, {waitForAck: true})
        }
        ...
        const katalog = (teks) => {
            res = PutraXd.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": -73926483, "message": teks, "footerText": "Its Me PutraXd Ofc ?\n𝑪𝒓𝒆𝒂𝒕𝒐𝒓 : PutraXd Ofc", "thumbnail": fakeimage, "surface": 'CATALOG' }}, {quoted:ftroli})
            PutraXd.relayWAMessage(res)
        }
        const cel =  {
             key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2021, status: 200, thumbnail: imagePutraXd, surface: 200, message: fake, orderTitle: 'PutraXd Ofc 𝘽𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩', sellerJid: '0@s.whatsapp.net'} } }       

        const ftroli =  {
             key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2021, status: 200, thumbnail: imgcel, surface: 200, message: fake, orderTitle: 'Its Me PutraXd Ofc ?\n𝑪𝒓𝒆𝒂𝒕𝒐𝒓 : PutraXd Ofc', sellerJid: '0@s.whatsapp.net'} } }       

        const fkon =  {
             key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2021, status: 200, thumbnail: fakeimage, surface: 200, message: PutraXdgamteng, orderTitle: 'Its Me PutraXd Ofc ?\n𝑪𝒓𝒆𝒂𝒕𝒐𝒓 : PutraXd Ofc', sellerJid: '0@s.whatsapp.net'} } }       
        ...
        const reply = (teks) => {
	      PutraXd.sendMessage(from, teks, text, {quoted:mek, thumbnail: fakeimage})
        }
        const sendMess = (hehe, teks) => {
           PutraXd.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? PutraXd.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/Menu.jpg')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : PutraXd.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/Menu.jpg')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           PutraXd.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return PutraXd.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/Menu.jpg')})
        }
       ...
       const kick = function(from, orangnya){
	       ...
	       PutraXd.groupRemove(from, [i])
        }
        }

       const kickMember = async(id, target = []) => {
           let group = await PutraXd.groupMetadata(id)
           let owner = group.owner.replace("c.us", "s.whatsapp.net")
           let me = PutraXd.user.jid
           for (i of target) {
           if (!i.includes(me) && !i.includes(owner)) {
           await PutraXd.groupRemove(to, [i])
        } else {
           await PutraXd.sendMessage(id, "Not Premited!", "conversation")
           break
        }
    }
}
       const add = function(from, orangnya){
	       PutraXd.groupAdd(from, orangnya)
}
       const sendKontak = (from, nomor, nama, org = "") => {
	       const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:' + org + '\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	       PutraXd.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: mek})
}
      const hideTag = async function(from, text){
	       let anu = await PutraXd.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       PutraXd.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Menu.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
      const sendWebp = async(to, url) => {
           ...
           PutraXd.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:mek})
           fs.unlinkSync(asw)
});
});
});
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           PutraXd.sendMessage(to, media, type, {quoted: mek, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       PutraXd.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       PutraXd.sendMessage(from, hasil, type, options).catch(e => {
	       PutraXd.sendMessage(from, { url : link }, type, options).catch(e => {
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           let g = await PutraXd.groupMetadata(to)
           let owner = g.owner.replace("c.us","s.whatsapp.net")
           let me = PutraXd.user.jid
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await PutraXd.groupMakeAdmin(to, [i])
      ...
      const demoteAdmin = async function(to, target=[]){
           ...
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await PutraXd.groupDemoteAdmin(to, [i])
           reply(`Hm..... @${mentioned[0].split('@')[0]} JABATANMU TURUNKAN SAYA IKUT SEDIH`)
				
}
}
}
          let authorname = PutraXd.contacts[from] != undefined ? PutraXd.contacts[from].vname || PutraXd.contacts[from].notify : undefined	
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
      ...
      return [h, m, s].map((v) => v.toString().padStart(2, 0)).join(':');
       ...
       console.error(err)
        ...
        const troli =  {key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 300, status: 200, thumbnail: fakeimage, surface: 200, message: fake, orderTitle: 'PutraXd', sellerJid: '0@s.whatsapp.net'} } }
        const ftext = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: { "extendedTextMessage": {"text": `*Hai ${pushname}👋*\n  ${moment().utcOffset('+0700').format('HH:mm:ss')} ${moment.tz('Asia/Jakarta').format('DD/MM/YYYY')}`,"title": `Hmm`,'jpegThumbnail': fs.readFileSync('./media/Menu.jpg')}}}
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/Menu.jpg`)},"title": `HALLO...${pushname}JANGAN LUPA DI ORDER`,"description": "PutraXd Botz KANG TOLOL", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "PutraXd-Botz","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}

      // Anti link
        if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (budy.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                reply(`*「 GROUP LINK DETECTOR 」*\n\nSepertinya kamu mengirimkan link grup, maaf kamu akan di kick`)
                PutraXd.groupRemove(from, [sender])
            }
        }
...
function banChat() {
    ...
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await PutraXd.setStatus(`Aktif Selama: ${uptime} | Mode: ${banChats ? 'SELF-MODE' : 'PUBLIC-MODE'}`).catch((_) => _);
      settingstatus = new Date() * 1;            
          }
    if (!mek.key.fromMe && banChats === true) return;
        // Sewa
             _sewa.expiredCheck(PutraXd, sewa)
      ...
      // GAME 
             game.cekWaktuFam(PutraXd, family100)
        ...
        if (game.isfam(from, family100)) {
            ...
            if (anjuy.length < 1){
                PutraXd.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
...
➸ *Sejak* : ${getTime}`
      mentions(cptl, x, true)
	    ...
	    // Auto Read
        PutraXd.chatRead(from, "read")
        //auto vn 
        await PutraXd.updatePresence(from, Presence.recording)
 ...
 case 'donasi':
               ...
               prep = await PutraXd.prepareMessageFromContent(from,{buttonsMessage},{})
               PutraXd.relayWAMessage(prep)
               break
             case 'bara': 
...
kma = gam1
mhan = await PutraXd.prepareMessage(from, kma, image)
...
PutraXd.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
        ...
        case 'help':               
               ...
               totalChat = await PutraXd.chats.all()
               groups = PutraXd.chats.array.filter(v => v.jid.endsWith('g.us'))
               privat = PutraXd.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
               ...
               imageMsg = (await PutraXd.prepareMessageMedia(fs.readFileSync(`./media/Menu.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/Menu.jpg`)})).imageMessage
...
`, imageMessage: imageMsg,
               ...
               prep = await PutraXd.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek, contextInfo:{ mentionedJid: parseMention(menu), externalAdReply: {
							title: `Hai ${pushname} 👋`,
							body: `${tanggal}`,
							previewType: 1,
        ...
        case 'command':
               list = []
               listmenu = [`groupmenu`,`wibumenu`,`stickermenu`,`islammenu`,`sertimenu`,`ceritamenu`,`makermenu`,`dewasamenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`owner`,`infomenu`,`othermenu`,`toolsmenu`,`gcputra`,`store`]
               listmenuu = [`Menu Group`,`Wibu Menu`,`Sticker Menu`,`Islam Menu`,`Serti Menu`,`Cerita Menu`,`Maker Menu`,`Dewasa Menu`,`Owner Menu`,`Game Menu`,`Fun Menu`,`Downloader`,`Owner Bot`,`Info Menu`,`MenuLainnya`,`Tools Menu`,`Official Group`,`Store/Jualan`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
Doing something
`,`Created By PutraXd Ofc              `, list, { quoted: freply})
               break
               case 'gcputra':
        ...
        case 'allmenu':
        case 'menuall':
               const boy = "6283842942635@s.whatsapp.net"     
...
┃⬡ Contact : @${tagme} 
┃⬡ Instagram :
┃⬡ *https://instagram.com/putragans167*
┃⬡ YouTube : 
┃⬡ https://youtube.com/channel/UCJGX0lPZWVDidVL2pEFDaSw
┃⬡ Official Group :
┃⬡ *https://chat.whatsapp.com/FYcSmBK4qVW3yvna8Yyzu3*
┃⬡ Rest Api :
┃⬡ *https://res-apiputra.herokuapp.com/*
┗⬣
...
//------------------< Store jualan >-------------------              
         case 'store':
               ...
               break
        case 'dmff':             
            PutraXd.sendMessage(from, dmff(prefix), MessageType.text, {quoted: ftroli})
            break       
       case 'editing':             
            PutraXd.sendMessage(from, editing(prefix), MessageType.text, {quoted: ftroli})
            break          
       case 'muridbot':             
            PutraXd.sendMessage(from, muridbot(prefix), MessageType.text, {quoted: ftroli})
            break     
       case 'pointft':             
            PutraXd.sendMessage(from, pointft(prefix), MessageType.text, {quoted: ftroli})
            break     
       case 'crff':             
            PutraXd.sendMessage(from, crff(prefix), MessageType.text, {quoted: ftroli})
            break     
       case 'webjualan':             
            PutraXd.sendMessage(from, webjualan(prefix), MessageType.text, {quoted: ftroli})
            break  
       case 'culikmem':       
            PutraXd.sendMessage(from, culikmem(prefix), MessageType.text, {quoted: ftroli})
            break     
      case 'follig':
            PutraXd.sendMessage(from, follig(prefix), MessageType.text, {quoted: ftroli})
            break
     case 'payment':
       ...
       SILAKAN KETIK MANUAL JIKA TIDAK TERLIHAT`
               ...
               imageMsg = (await PutraXd.prepareMessageMedia(fs.readFileSync(`./media/wpmobile.png`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/Menu.jpg`)})).imageMessage
               ...
               prep = await PutraXd.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftroli})
               PutraXd.relayWAMessage(prep)
               break
      case 'gopay':
      ...
      case 'qris':
             ...
             teks = `SCAN QRIS DI ATAS A/N PutraXdBOTZ`
             PutraXd.sendMessage(from, wew, image, { quoted: mek, caption: teks })
             break 
//------------------< menu nye >-------------------      
case 'groupmenu':
       ...
       *────❲GRUP MENU❳────*
       ...
       ▢ ${prefix}linkgc`
               ...
               buttonsMessage = {
               contentText: `${menu}`,
               footerText: 'Created By PutraXd Ofc', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
               ...
               PutraXd.sendMessage(from, starGame, text, {quoted: mek, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
       ...
       case 'tebakanime':
              ...
              PutraXd.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*+* ```Tebak Anime```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s' }).then(() => {
       ...
       case 'tebaklagu':
              ...
              reply('*+* ```Tebak Lagu```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s')
              PutraXd.sendMessage(from, ini_buffer, audio, {quoted: mek}).then(() => {
       ...
       case 'tebaktebakan':
              ...
              PutraXd.sendMessage(from, '*+* ```Tebak Tebakan```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'kuismath':
              ...
              jawaban = get_result.jawaban
              ini_buffer = await getBuffer(ini_image)
              PutraXd.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*+* ```Kuis Matematika```\n\nSilahkan jawab soal berikut ini\n\n• *Waktu* : 50s' }).then(() => {
       ...
       case 'tebakgambar':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              ini_buffer = await getBuffer(ini_image)
              PutraXd.sendMessage(from, ini_buffer, image, { quoted: mek, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :'+kisi_kisi+'\nWaktu : 30s' }).then(() => {
       ...
       case 'siapaaku':
              ...
              PutraXd.sendMessage(from, '*+* ```Tebak Siapakah Aku```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'tebakkata':
              ...
              PutraXd.sendMessage(from, '*+* ```Tebak Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'tebaklirik':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              PutraXd.sendMessage(from, '*+* ```Tebak Lirik```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
      ...
      case 'tebakjenaka':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              PutraXd.sendMessage(from, '*+* ```Tebak Jenaka```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'kimiakuis':
              ...
              PutraXd.sendMessage(from, '*+* ```Tebak Kimia```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'tebakbendera':
              ...
              PutraXd.sendMessage(from, '*+* ```Tebak Bendera```\n\n• *Bendera* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'susunkata':
              ...
              jawaban = get_result.jawaban
              pertanyaan = get_result.pertanyaan
              PutraXd.sendMessage(from, '*+* ```Susun Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'asahotak':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.pertanyaan
              PutraXd.sendMessage(from, '*+* ```Asah Otak```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'caklontong':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              PutraXd.sendMessage(from, '*+* ```Caklontong```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
...
HARGA PERMANEN:~15k~ PROMO!!! *10K* MINAT? HUBUNGI OWNER`
              PutraXd.sendMessage(from, await getBuffer(gopeynya), image, {quoted: mek, caption: teksnya })
              break             
//------------------< bayar menu >-------------------  
case 'bayar':
gopeynya = 'https://telegra.ph/file/5300c74e3b843b9c0d29d.jpg'
...
• Subrek PutraXd Botz
━━━━━━━━━━━━━━━━━━━━`
        PutraXd.sendMessage(from, await getBuffer(gopeynya), image, {quoted: ftoko, caption: teksnya })
              break
...
*き⃟🦈 ${prefix}nekopoisearch*`
              PutraXd.sendMessage(from, await getBuffer(gopeynya), image, {quoted: ftext, caption: teksnya })
              break
case 'buttonown':
              gopeynya = 'https://telegra.ph/file/5300c74e3b843b9c0d29d.jpg'
              teksnya = `*「OWNER MENU」*
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case 'google':
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               PutraXd.sendMessage(from, ss, image, {caption: vars, quoted : mek})
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
})
...
_*Tunggu Proses Upload Media......*_`
      ...
      case 'tiktoknowm':
            ...
            .catch((err) => { reply(String(err)) })
             break
case 'tiktok':
              if (!isRegister) return reply(`You are not verified\n\nReply this chat and send bot password\n\nHint : \nPassword contains 4 digit number\nCheck password at: https://PutraXd-chan02.github.io`)
              if (!q) return reply('Linknya?')
              if (!q.includes('tiktok')) return reply(mess.error.Iv)
              ...
              imageMsg = (await PutraXd.prepareMessageMedia(fs.readFileSync(`./media/Menu.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/Menu.jpg`)})).imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Subscribe Yt PutraXd-Botz\n Helpme tu 1k subscriber', imageMessage: imageMsg,
              contentText:`Silahkan pilihan media yg mau di download kak:v`,buttons,headerType:4}
              prep = await PutraXd.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              PutraXd.relayWAMessage(prep)
              break
      case 'ttaudio': 
      case 'tiktokmusic': 
      case 'tiktokaudio':
             ...
             PutraXd.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
             break
      case 'fb':
      case 'facebook':
             if (!q) return
      ...
      case 'ssweb':
             ...
             await PutraXd.sendMessage(from, ini_buffer, image, { quoted: mek })
             break
       case 'nhentaipdf':
             if (!isPremium) return reply(mess.only.premium)
             ...
             PutraXd.sendMessage(from, ini_image, image, { caption: teks, quoted: mek })
             anu = await fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${henid}?apikey=${setting.lolkey}`)
             pdf = await getBuffer(anu.result)
             PutraXd.sendMessage(from, pdf, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${get_result.title_romaji}.pdf`, thumbnail: ini_image })
             } catch (e) {
             console.log(e)
             reply(String(e))
}
       ...
       case 'manga':
              ...
              await PutraXd.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
              break
       case 'doujindesu':
             if (!q) return reply(mess.wrongFormat)
             reply(mess.wait)
             ...
             PutraXd.sendMessage(from, buffer, image, {caption: xixixai, quoted: mek})
             } catch (e) {
             console.log(e)
             reply(String(e))
}
      ...
      case 'kusonime':
             ...
             await PutraXd.sendMessage(from, ini_buffer, image, { quoted: mek, caption: ini_txt })
             break
       case 'otakudesu':
              if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
              reply(mess.wait)
       ...
       case 'wallnime':
              ...
              PutraXd.sendMessage(from, gambar, image, { quoted: mek, thumbnail: Buffer.alloc(0) })
})
              break
       
       case 'hentai':
              getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/hentai?apikey=${setting.lolkey}`).then((gambar) => {
              PutraXd.sendMessage(from, gambar, image, { quoted: mek })
}) 
              break
       case 'storyanime':
              reply(mess.wait)
              ...
              PutraXd.sendMessage(from, buffer, video, { quoted: mek })
              break
       case 'nekopoi3d':
       case '3dnekopoi':
       case '3dnekopoilast':
        ...
        case 'otakuongoing':              
               ...
               PutraXd.sendMessage(from, buff, image, {quoted: mek, caption: ot})
               break
            case 'waifu':
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              imageMsg = ( await PutraXd.prepareMessage(from, inifile, 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
...
break
       ...
       case 'wallml':
		      ...
		      buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              imageMsg = ( await PutraXd.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Donasi Ya Kak ☕', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await PutraXd.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              PutraXd.relayWAMessage(prep)
...
*[Wait] Tunggu Sebentar..*`
				...
				PutraXd.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Nih Gan'})
					break  
				case 'play':
              if (!q) return reply('Linknya?')
...
\`\`\`き⃟🦈 Link : ${get_data[i].url}\`\`\``
              ...
              PutraXd.sendMessage(from, ini_buffer, image, { quoted: mek, caption: teks })
              } catch {
...
\`\`\`き⃟🦈 Link : ${i.url}\`\`\``
       ...
       case 'tourl':
               ...
               owgi = await PutraXd.downloadMediaMessage(boij)
       ...
       case 'img2url':
               ...
               var media = await  PutraXd.downloadAndSaveMediaMessage(encmedia)       
               imgbb('39d895963468b814fad0514bd28787e2', media)
              .then(data => {
               var caps = `*_IMAGE TO URL_*\n\n*~>  ID :* ${data.id}\n*~>  MimeType :* ${data.image.mime}\n*~>  Extension :* ${data.image.extension}\n*~>  URL :* ${data.display_url}`
               ibb = fs.readFileSync(media)
               PutraXd.sendMessage(from, ibb, image, { quoted: mek, caption: caps})
})
              .catch(err => {
               throw err
})
         ...
         case 'asupan': // shansekai                
              ...
              .then(async (body) => {
               ...
               sendMediaURL(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, '\`\`\`ASUPAN BRO GUA DAPAT DARI LORD ©PutraXd Botz\`\`\`')
        ...
        case 'tulis':
               ...
               PutraXd.sendMessage(from, buff, image, {quoted: mek, caption: mess.success}).catch(e => {
...
//------------------< Level >-------------------
      case 'level': 
              ...
              try {
              profilePic = await PutraXd.getProfilePicture(sender)
              } catch {
              profilePic = errorImg
}
              ...
              PutraXd.sendMessage(from, buffer, image, { caption: teks, quoted: mek})
              break
...
//------------------< Stalk >-------------------
      case 'stalkgithub':
      case 'githubstalk':
              if (args.length == 0) return reply(`Example: ${prefix + command} PutraXd-chan02`)
              reply(mess.wait)
              username = args[0]
...
`
             PutraXd.sendMessage(from, ini_buffer, image, { caption: ini_txt, thumbnail: Buffer.alloc(0) })
             break
      case 'stalkig':
      case 'igstalk':
             if (args.length == 0) return reply(`Example: ${prefix + command} PutraXd.chan26`)
             reply(mess.wait)
             username = args[0]
...
`
              PutraXd.sendMessage(from, pp_tt, image, { quoted: mek, caption: ini_txt, thumbnail: Buffer.alloc(0) })
              break
       case 'iguser':
              try {
...
//------------------< Sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'telestiker':
              ...
              PutraXd.sendMessage(from, ini_buffer, sticker, {})
}
              break
       case 'semoji':
       case 'emoji':
              ...
              await PutraXd.sendMessage(from, ini_buffer, sticker, { quoted: mek })
              break
case 'ttp':
...
break
       case 'attp':
              ...
              PutraXd.sendMessage(from, buffer, sticker, { quoted: mek })
              break
       case 'ttg':
       ...
       case 'gifstiker':
            ...
            const media = await PutraXd.downloadAndSaveMediaMessage(encmedia)
                ran = '666.webp'
                await ffmpeg(`./${media}`)
                .input(media)
                ...
                .on('end', function () {
                console.log('Finish')
                PutraXd.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
       ...
       case 'colong':
              ...
              media = await PutraXd.downloadAndSaveMediaMessage(encmedia)
              anu = args.join(' ').split('|')
              satu = anu[0] !== '' ? anu[0] : `${pushname}`
              dua = typeof anu[1] !== 'undefined' ? anu[1] : `UwU`
              require('./lib/fetch.js').createExif(satu, dua)
              require('./lib/fetch.js').modStick(media, PutraXd, mek, from)
              break
       case 'delwm':
              if (!isQuotedSticker) return reply('Stiker aja om')
       ...
       case 'swm':
             ...
             .on('error', (e) => {
              console.log(e)
              PutraXd.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: mek })
              fs.unlinkSync(media)
})
             .on('end', () => {
            _out = getRandom('.webp')
             ...
             .on('exit', () => {
              PutraXd.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: mek })
              fs.unlinkSync(out)
              fs.unlinkSync(_out)
              fs.unlinkSync(media)
})
      ...
      case 'toimg':
              ...
              PutraXd.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
       case 'smeme': 
...
owgi = await  PutraXd.downloadAndSaveMediaMessage(ger)
...
break
       ...
       case 'memegen':                    
        ...
        case 'togif':
               ...
               mediaaa = await PutraXd.downloadAndSaveMediaMessage(encmediaaa)
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               PutraXd.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: mek, caption: mess.success})
        ...
        case 'tovideo':
               ...
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               PutraXd.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: mek, caption: mess.success})
               fs.unlinkSync(mediaaa)
               } else {
               reply(mess.wrongFormat)
}
        ...
        case 'tomp3':
               ...
               PutraXd.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
               fs.unlinkSync(ran)
})
               } else {
               reply(mess.wrongFormat)
...
//------------------<18+ Menu>-----------------------   
       case 'randombokep':
              ...
              PutraXd.sendMessage(from, '*PERMINTAAN:* '+bokep+'\n*DOSA TANGGUNG PRIBADI*\n*NI BRO FREE BUAT KAMU DOWNLOAD SENDIRI:* '+ kep, text, { quoted: ftoko, caption: `NI BOKEP SAYA DAPAT DARI *©PutraXd Botz* DOSA TANGGUNG SENDIRI🗿`})
              break
                case 'xnxx':
                    ...
                    PutraXd.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    break
        case 'xnxxsearch':
                    ...
                    break 
case 'asupan1':
			PutraXd.updatePresence(from, Presence.composing) 
				reply(mess.wait)
				...
				PutraXd.sendMessage(from, asupan, video, {quoted: mek, caption: '```ASUPAN NIH:V```'})
				break        
case 'asupan2':
			PutraXd.updatePresence(from, Presence.composing) 
				reply(mess.wait)
				...
				qute = fs.readFileSync('media/Menu.jpg') 
				   PutraXd.sendMessage(from, qute, image, { quoted: freply, caption: '*SEMOGA DI BERI HIDAYAH*\nLink Download \n\nhttps://www.mediafire.com/file/h2nygxbyb6n9cyo/VID-20210107-WA1468.mp4/file' })
				   break
				   case 'dosa2':
...
PutraXd.sendMessage(from, tapib1, document, { quoted: mek, filename:`PutraXd-Botz ~ 404 ${vipi}.mp3`, mimetype: 'audio/application' })
await reply('Bang mau nanya')
break  
case 'pgp':
if (!isOwner && !mek.key.fromMe) return
buf = Mfake
imeu = await PutraXd.prepareMessage('0@s.whatsapp.net', buf, image) 
imeg = imeu.message.imageMessage
res = await PutraXd.prepareMessageFromContent(from,{
"groupInviteMessage": {
"groupJid": "85296556573-1328272333@g.us",
"inviteCode": "wFHwtOxGQN8OwK2x",
"inviteExpiration": "162533333338378",
"groupName": `PutraXd-Botz ~ 404 ${vipi}`,
...
PutraXd.toggleDisappearingMessages(from, 'Awoakwoakwoak')
PutraXd.relayWAMessage(res)
break
case 'plp':
res = await PutraXd.prepareMessageFromContent(from,{
...
\`\`\`き⃟🦈 Wa Version: ${PutraXd.user.phone.wa_version}\`\`\`
\`\`\`き⃟🦈 Os Version: ${PutraXd.user.phone.os_version}\`\`\`
\`\`\`き⃟🦈 Device Manufacturer: ${PutraXd.user.phone.device_manufacturer}\`\`\`
\`\`\`き⃟🦈 Device Model: ${PutraXd.user.phone.device_model}\`\`\`
\`\`\`き⃟🦈 Os Build Number: ${PutraXd.user.phone.os_build_number}\`\`\``
             reply(teks)
             break  
//------------------< Owner >-------------------
      case 'addupdate':
...
case 'tupai':
				reply('PROSES')
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
						...
						PutraXd.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: troli})
						fs.unlinkSync(ran)
					})
				break 
case 'addstik':
		if (!isOwner) return reply (mess.only.owner)
				...
				delb = await PutraXd.downloadMediaMessage(boij)
				setiker.push(`${svst}`)
				fs.writeFileSync(`./temp/stick/${svst}.webp`, delb)
				fs.writeFileSync('./temp/stick.json', JSON.stringify(setiker))
				PutraXd.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}liststik`, MessageType.text, { quoted: troli})
				break
case 'addimg':
				if (!isOwner) return reply (mess.only.owner)
...
case 'liststik':
				...
				PutraXd.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": setiker } })
				break				
case 'listimg':
				teks = '*Image list :*\n\n'
...
case 'listvid':
				...
				PutraXd.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": imagenye } })
				break				
case 'listvn':
				teks = '*List Vn:*\n\n'
				...
				PutraXd.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": audionye } })
				break	
case 'getstik':
				namastc = body.slice(9)
				try {
				result = fs.readFileSync(`./temp/stick/${namastc}.webp`)
				PutraXd.sendMessage(from, result, sticker,{quoted:troli})
				} catch {
...
case 'getimg':
				...
				PutraXd.sendMessage(from, buffer, image, { quoted: troli, caption: `Result From Database : ${namastc}.jpeg` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break
...
case 'getvid':
				...
				PutraXd.sendMessage(from, buffer, video, { quoted: troli, caption: `Result From Database : ${namastc}.mp4` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break				
case 'getvn':
				...
				PutraXd.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: troli, ptt: true })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break										      
      case 'join': 
             ...
             fak = PutraXd.query({ json: ['action', 'invite', link],
             expect200: true })
      ...
      case 'bc':
             ...
             anu100 = await PutraXd.chats.all()
             if (isMedia && !PutraXd.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(PutraXd).replace('quotedM','m')).message.extendedTextMessage.contextInfo : PutraXd
             bc100 = await PutraXd.downloadMediaMessage(encmedia)
             for (let _ of anu100) {
             PutraXd.sendMessage(_.jid, bc100, image, {quoted: freply, caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu100) {
             PutraXd.sendMessage(_.jid, 
			{"contentText": `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`,
			"footerText": 'PutraXd Ofc☕︎',
			"buttons": [
			{"buttonId": `#menu`,
      ...
      case 'clearall':
             if (!isOwner) return  reply(mess.only.owner)
             anu = await PutraXd.chats.all()
             PutraXd.setMaxListeners(25)
             for (let _ of anu) {
             PutraXd.deleteChat(_.jid)
}
      ...
      case 'leaveall':
             if (!isOwner) return  reply(mess.only.owner)
             let totalgroup = PutraXd.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
             for (let id of totalgroup) {
             sendMess(id, 'Byee', null)
             await sleep(3000)
             PutraXd.groupLeave(id)
}
             break
//------------------< G R U P >-------------------
case 'linkgc':
				if (!isGroup) return reply(mess.only.group)
				linkgc = await PutraXd.groupInviteCode (from)
				yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
				PutraXd.sendMessage(from, yeh, text, {quoted: mek})
				break
            case 'kick':
      ...
      case 'promote':
             ...
             PutraXd.groupMakeAdmin(from, mems_ids)
             } else {
             PutraXd.groupMakeAdmin(from, entah)
}
             } else {
             entah = mek.message.extendedTextMessage.contextInfo.participant
             PutraXd.groupMakeAdmin(from, [entah])
      ...
      case 'demote':
             ...
             PutraXd.groupDemoteAdmin(from, mems_ids)
             } else {
             PutraXd.groupDemoteAdmin(from, [entah[0]])
}
             } else {
             entah = mek.message.extendedTextMessage.contextInfo.participant
             PutraXd.groupDemoteAdmin(from, [entah])
}
             break
       case 'setgrupname':
              if (!isGroupAdmins) return reply(mess.only.admin)
              ...
              PutraXd.groupUpdateSubject(from, q)
       ...
       case 'setdesc':
              ...
              PutraXd.groupUpdateDescription(from, q)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              break
       case 'setppgrup':
              ...
              let media = await PutraXd.downloadMediaMessage(encmedia)
              PutraXd.updateProfilePicture(from, media)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              } else {
       ...
       case 'profile':
              PutraXd.updatePresence(from, Presence.composing)
              try {
              profil = await PutraXd.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
              } catch {
              profil = errorImg
}
              thu = await PutraXd.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
              me = PutraXd.user
              uptime = process.uptime()
              profile = `-----[ *USER INFO* ]-----\n\n➸ *Username:* ${pushname}\n➸ *Status:* ${thu.status}\n➸ *Premium*: ${isPremium ? 'Ya' : 'No'}\n➸ *Admin*: ${isGroupAdmins ? 'Ya' : 'No'}\n➸ *Prefix :* Multi Prefix\n\n=_=_=_=_=_=_=_=_=_=_=_=_=\n\nYour progress:\n➸ *Level*: -\n➸ *XP*: ${Xpluu} / ${requiredXplu}`
              buff = await getBuffer(profil)
              PutraXd.sendMessage(from, buff, image, {quoted: freply, caption: profile})
              break
       case 'afk': 
              if (!isGroup) return reply(mess.only.group)
       ...
       case 'groupinfo':
              if (!isGroup) return reply(mess.only.group)
              try {
              var pic = await PutraXd.getProfilePicture(from)
              } catch {
              var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
              let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n${groupMetadata.desc}`
              PutraXd.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
              break
       case 'tagall':
              if (!isGroupAdmins) return reply(mess.only.admin)
              if (!isGroup) return reply(mess.only.group)
       ...
       case 'here':                
             ...
             let online = [...Object.keys(PutraXd.chats.get(ido).presences), PutraXd.user.jid]
             PutraXd.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      ...
      case 'sider':
             ...
             if(!isGroup) return reply(mess.only.group)
             try {
             infom = await PutraXd.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
             tagg = []
             teks = `*• Dibaca oleh:*\n\n`
    ...
    case 'public':
       ...
       case 'toxic':
        ...
        case 'citacita':
              ...
              PutraXd.sendMessage(from, cita2, audio,{mimetype: 'audio/mp4', ptt:true, quoted: mek})
              break
       case 'apakah':
              apakah = body.slice(1)
              const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi']
              const kah = apa[Math.floor(Math.random() * apa.length)]
              PutraXd.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: mek })
              break
       case 'rate':
       case 'nilai':
              rate = body.slice(1)
              ...
              PutraXd.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: mek })
              break
       case 'gantengcek':
       case 'cekganteng':
              ...
              PutraXd.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
              break
       case 'cantikcek':
       case 'cekcantik':
              cantik = body.slice(1)
              ...
              PutraXd.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
              break
       case 'cekwatak':
              var namao = pushname
              var prfx = await PutraXd.getProfilePicture(sender)
       ...
       case 'hobby':
              ...
              PutraXd.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
              break
       case 'bisakah':
              bisakah = body.slice(1)
              const bisa =['Bisa','Tidak Bisa','Coba Ulangi','MANA GW TAU']
              const keh = bisa[Math.floor(Math.random() * bisa.length)]
              PutraXd.sendMessage(from, '*Pertanyaan :* '+bisakah+'\n*Jawaban :* '+ keh, text, { quoted: mek })
              break
       case 'kapankah':
              kapankah = body.slice(1)
              const kapan =['Besok','Lusa','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi']
              const koh = kapan[Math.floor(Math.random() * kapan.length)]
              PutraXd.sendMessage(from, '*Pertanyaan :* '+kapankah+'\n*Jawaban :* '+ koh, text, { quoted: mek })
              break
       case 'truth':
              ...
              PutraXd.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
              break
       case 'dare':
              ...
              PutraXd.sendMessage(from, buffer, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
              break		
       case 'jadian':
              jds = []
              jdii = groupMembers
...
//------------------< Lainnya >-------------------
        ...
        case 'getpp':
               if (mek.message.extendedTextMessage === null || mek.message.extendedTextMessage === undefined) {
               linkpp = await PutraXd.getProfilePicture(from) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               PutraXd.sendMessage(from, buffer, image, {caption: "Nih", quoted: mek })
               } else if (mek.message.extendedTextMessage.contextInfo.mentionedJid === null || mek.message.extendedTextMessage.contextInfo.mentionedJid === undefined) {
               mberr = mek.message.extendedTextMessage.contextInfo.participant
               linkpp = await PutraXd.getProfilePicture(mberr) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               PutraXd.sendMessage(from, buffer, image, { quoted: mek, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
        ...
        case 'delete': // MR.CYSER
               ...
               PutraXd.deleteMessage(from, {id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true})
               } catch (e){
               reply('Reply chat bot')
}
               break
        ...
        case 'info':  // Jangan Di Ubah Plise
             ...
             PutraXd.sendMessage(from, await getBuffer(urlinfo), image, {quoted: mek, caption: thankslort })
             break
      case 'get':
...
case 'caripesan':  //by ANU TEAM
             ...
             reply(mess.wait)
             cok = await PutraXd.searchMessages(`${ve}`, from, batas,1) 
             if (cok.messages.length < 2) return reply('Tidak Ditemukan Pesan') 
             if (cok.messages.length < parseInt(batas)) reply(`Hanya Ditemukan ${cok.messages.length - 1} Pesan`)
             for (i=1;i < cok.messages.length;i++) {
             if (cok.messages[i].message) {
             PutraXd.sendMessage(from, `Ditemukan!`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
             } catch (e) {
       ...
       case 'cekapikey':
              ...
              PutraXd.sendMessage(from, teks, text, {quoted: troli})
              break
       case 'bugreport':
              if (args.length < 1) return reply(`Ketik ${prefix}bugreport [fiturnya] [Error Nya Gimana]`) 
              teks = args.join(' ')
              reply('Terima Kasih Telah Melaporkan Bug Pada Owner, Jika Itu Sekedar Iseng Maka Akan Di Ban Oleh Bot!')
              PutraXd.sendMessage('6283842942635@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
              totalchat.map( async ({ jid }) => {
              await PutraXd.chatRead(jid)
})
...
case 'asmaulhusna':
 ...
 case 'alquranaudio':
                    ...
                    PutraXd.sendMessage(from, ini_buffer, audio, { quoted: mek, mimetype: Mimetype.mp4Audio })      
                    break                                         
//------------------< serti menu >-------------------   
case 'sertitolol': 				
				...
				PutraXd.sendMessage(from, zhain, image, { quoted: mek, caption: 'Nih Dah Jadi' })
				break
//------------------< cerita menu >-------------------
case 'cerpen':
...
PutraXd.version = [2, 2119, 6]
PutraXd.browserDescription = ['PutraXd','Desktop','3.0']
if (args[0] && args[0].length > 200) {
	let json = Buffer.from(args[0], 'base64').toString('utf-8')
    let obj = JSON.parse(json)
    await PutraXd.loadAuthInfo(obj)
}
try {
PutraXd.on('qr' ,async qr => {
...
let scen = await PutraXd.sendMessage(from, fs.readFileSync(`./jadibot@${sender}.jpg`), MessageType.image, {quoted : mek,caption: 'Scan QR ini untuk jadi bot sementara!\n1. Klik titik tiga di pojok kanan atas\n2. Ketuk WhatsApp Web\n3. Scan QR ini \n\nQR Expired dalam 20 detik'})    
setTimeout(() => {
       PutraXd.deleteMessage(from, scen.key)
  }, 30000);
})  
PutraXd.on ('open', async () => {
  console.log ('credentials update')
  const authInfo = PutraXd.base64EncodedAuthInfo()
  fs.writeFileSync(`./sampah/${sender}.json`, JSON.stringify(authInfo  ,null, '\t'))
  await PutraXd.sendMessage('0@s.whatsapp.net', `Kamu bisa login tanpa qr dengan pesan dibawah ini`, MessageType.extendedText)
  PutraXd.sendMessage('0@s.whatsapp.net', `${prefix}${command} ${Buffer.from(JSON.stringify(authInfo)).toString('base64')}`, MessageType.extendedText)
})
PutraXd.on('chat-update', async (chat) => {
	require('./PutraXd.js')(PutraXd, chat)
})    
await PutraXd.connect().then(async ({user}) => {
...
case 'coffe':          
        ...
        PutraXd.sendMessage(from, gambar, image, { quoted: troli, caption : 'jangan lupa bayar 15.000' })
                    })
                    break
                case 'cup1':        
                    if (args.length == 0) return reply(`Example: ${prefix + command} LoL Human`)
                ...
                case 'freefire':
                    ...
                    PutraXd.sendMessage(from, gambar, image, { quoted: ftoko })
                    })
                    break                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       
        ...
        case 'groupsetting':
 		...
 		case 'group':
					...
					if (args[0] === 'buka') {
						...
						PutraXd.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`*BERHASIL MENUTUP GROUP*`)
						PutraXd.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break                            
//------------------< Menunya Bang:v >-------------------
            case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya :\n https://instagram.com/putragans167`)
             break
      case 'infoyt':
            reply(`Jangan Lupa Subscribe YouTube Owner Ya :\n https://youtube.com/channel/UCJGX0lPZWVDidVL2pEFDaSw`)
            break
      case 'grupbot':
             reply('')
             break
      case 'ownermenu':
             PutraXd.sendMessage(from, ownerMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'downloadmenu':
             PutraXd.sendMessage(from, downloadMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'gamemenu':
             PutraXd.sendMessage(from, gameMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'rules':
             PutraXd.sendMessage(from, rulesBot(prefix), MessageType.text, {quoted: troli})
             break
       case 'owner':
             PutraXd.sendMessage(from, owner(prefix), MessageType.text, {quoted: troli})
             break
      case 'wibumenu':
             PutraXd.sendMessage(from, wibuMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'infomenu':
             PutraXd.sendMessage(from, infoMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'stickermenu':
             PutraXd.sendMessage(from, stickerMenu(prefix), MessageType.text, {quoted: troli})
             break
       case 'islammenu':
             PutraXd.sendMessage(from, islamMenu(prefix), MessageType.text, {quoted: troli})
             break
       case 'sertimenu':
             PutraXd.sendMessage(from, sertiMenu(prefix), MessageType.text, {quoted: troli})
             break
       case 'ceritamenu':
             PutraXd.sendMessage(from, ceritaMenu(prefix), MessageType.text, {quoted: troli})
             break  
       case 'makermenu':
             PutraXd.sendMessage(from, makerMenu(prefix), MessageType.text, {quoted: troli})
             break 
     case 'toolsmenu':
              PutraXd.sendMessage(from,toolsMenu(prefix), MessageType.text, {quoted: troli})
              break                   
      case 'dewasamenu':
             PutraXd.sendMessage(from, dewasaMenu(prefix), MessageType.text, {quoted: troli})
             break                                    
      case 'othermenu':
             PutraXd.sendMessage(from, otherMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'groupmenu': 
      case 'grupmenu': 
             PutraXd.sendMessage(from, groupMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'funmenu':
             PutraXd.sendMessage(from, funMenu(prefix), MessageType.text, {quoted: troli})
             break
...
• Pemenangnya adalah [ @${winR} ] `
	   PutraXd.sendMessage(from, starGame, text, {quoted: troli, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
		fs.unlinkSync("./media/" + from + ".json");
	} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
		PutraXd.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
...
Giliran = @${tty.player1.split('@')[0]}
  ...
  PutraXd.sendMessage(from, ucapan, text, {quoted: troli, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
PutraXd.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:troli,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
PutraXd.sendMessage(from, ucapan1, text, {quoted: troli, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
PutraXd.sendMessage(from, ucapan1, text, {quoted:troli, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
if (!mek.key.fromMe && !isOwner) return
try {
return PutraXd.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
PutraXd.sendMessage(from, {text:teks, jpegThumbnail:fs.readFileSync('./media/wpmobile.png')}, 'extendedTextMessage', {sendEphemeral:true, quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
}
	} 
